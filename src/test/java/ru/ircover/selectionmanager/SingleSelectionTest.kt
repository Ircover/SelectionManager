...
class SingleSelectionTest {
    private lateinit var selectionManager: SingleSelection

    @BeforeEach
    fun setup() {
        selectionManager = SingleSelection()
    ...
    fun isPositionSelected() {
        ...
        assertFalse(selectionManager.isPositionSelected(targetPosition1),
                "position $targetPosition1 wasn't pointed, but selected")
    }
    ...
    fun registerSelectionChangeListener() {
        ...
        val registrationDisposable = selectionManager.registerSelectionChangeListener { position, isSelected ->
            when(position) {
                targetPosition1 -> {
                    isTargetPosition1Processed = true
                    assertFalse(isSelected,
                            "position $targetPosition1 should be deselected")
                }
                targetPosition2 -> {
